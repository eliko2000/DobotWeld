...
local gTcpReadBuffer={}
for i=1,173 do gTcpReadBuffer[i]=0 end
...
local function innerReadData(self)
    ...
    while endTime-begTime<=3000 do
        err,recvData = TCPRead(self.sock, 1)
        if err~=0 then
            ...
            if gTcpReadBufferCount>=173 then
...
local function innerSetRobotPose(self, pose)
    ...
    for i=43,66 do --机器人发送的位置坐标
        idx = self.robotPoseCmd[i-42]
        self.packAllCmd[i] = idx&0xFF
    end
end
---------------------------------------------------------------------------------
---------------------------------------------------------------------------------
--【导出接口】
local IntelligenLaserPositionProtocol = {
...
---------------------------------------------------------------------------------
---------------------------------------------------------------------------------
function IntelligenLaserPositionProtocol.connect(sock)
    IntelligenLaserPositionProtocol.sock = sock
    innerBuildCmd(IntelligenLaserPositionProtocol) --初始化填充所有字节
    return true
end

function IntelligenLaserPositionProtocol.disconnect()
    IntelligenLaserPositionProtocol.sock = nil
end
...
function IntelligenLaserPositionProtocol.openLaser()
    local self = IntelligenLaserPositionProtocol
    self.cmd = 0xF1
    innerBuildCmd(self)
...
function IntelligenLaserPositionProtocol.closeLaser()
    local self = IntelligenLaserPositionProtocol
    self.cmd = 0xF2
    innerBuildCmd(self)
    if not innerSendData(self) then
...
function IntelligenLaserPositionProtocol.setTaskNumber(iNumber)
    local self = IntelligenLaserPositionProtocol
...
说明：这个值可以直接使用，机器人可以直接移动到此位置
]]--
function IntelligenLaserPositionProtocol.getRobotPosePosition(robotPose)
    local self = IntelligenLaserPositionProtocol
...
参数：robotPose-机器人当前(激光照射)坐标位置，值为：{x=1,y=1,z=1,rx=1,ry=1,rz=1}
返回值：成功返回坐标值{x=1,y=1,z=1,rx=1,ry=1,rz=1}，xyz单位mm,Rxyz单位是角度deg，失败返回nil
说明：这个值不一定可以直接使用，因为没有做滤波，需要验证
]]--
function IntelligenLaserPositionProtocol.getRobotPoseTrack(robotPose)
    local self = IntelligenLaserPositionProtocol
    
    innerSetRobotPose(self, robotPose)
    self.packAllCmd[2] = innerGetNextIdx(self)&0xFF --数据包编号
    self.packAllCmd[4] = 0x35 --指令字节 --self.cmd = 0x35
...
return IntelligenLaserPositionProtocol