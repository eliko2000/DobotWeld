...
function MqttRobotProtocol:connect(connectId,strLastWillTopic,strLastWillMsg)
    local function pfnExecute(self, connectId)
        ...
        MyWelderDebugLog("MQTTConnect:"..tostring(ret))
        self.connectId = connectId
    end
    local isOk,err = pcall(pfnExecute, self, connectId)
    if not isOk then
        MyWelderDebugLog("MQTTConnect fail:"..tostring(err))
    end
...
function MqttRobotProtocol:disconnect()
    if self.connectId then
        ...
        if not isOk then
            MyWelderDebugLog("MQTTDisconnect fail:"..tostring(err))
        end
        self.connectId = nil
    end
end
...
function MqttRobotProtocol:publish(strTopic, strMsg)
    ...
    if not isOk then
        ...
        MyWelderDebugLog("MQTTPublish fail:"..tostring(err))
        return false
    end
    return true
end
...
function MqttRobotProtocol:subscribe(strTopic)
    ...
    if not isOk then
        ...
        MyWelderDebugLog("MQTTSubscribe fail:"..tostring(err))
        return nil
    end
    return err
end
...
function MqttRobotProtocol:subscribeWait(strTopicArray,callbackName)
    ...
    if not isOk then
        self:disconnect()
        MyWelderDebugLog("MQTTWait fail:"..tostring(err))
        return false
    end
    return err
end
...
local function enterLock()
    Lock("ThreadSafeMqttLocker-C6868E5C-2B50-480E-9C43-AAF29D79D4DD",8000,10000)
end
local function leaveLock()
    UnLock("ThreadSafeMqttLocker-C6868E5C-2B50-480E-9C43-AAF29D79D4DD")
end

local function innerConnectIf(obj)
...
local function innerExecutePublish(obj,strTopic, msg)
    if type(msg)=="table" then msg = jsonLuaUtils.encode(msg)
...
local MqttRobot = {
    mqtt = nil,
    connectId = "Weld",
    lastWillTopic = "WeldPluginCrash",
    lastWillMsg = "welder process has exit or crash",
    cbSubscribeNameFlag = "__pfnCbExecHttpSubscribeWeldPlugin"
}

--此函数一定是全局唯一的
function __pfnCbExecHttpSubscribeWeldPlugin(strTopic, strMsg)
    if type(MqttRobot.pfnCBSunscribe)=="function" then
        local _ok,msg = pcall(MqttRobot.pfnCBSunscribe,strTopic, strMsg)
        if not _ok then MyWelderDebugLog(msg) end
    end
end
...
function MqttRobot.publish(strTopic, msg)
    ...
    if not isOk then
        MyWelderDebugLog("publish fail:"..tostring(strTopic).."->"..tostring(errmsg))
    end
    return isOk
end
...
function MqttRobot.subscribe(strTopicArray, pfnCallback)
    ...
    local isOk,errmsg = pcall(innerExecuteSubscribe,MqttRobot,strTopicArray, MqttRobot.cbSubscribeNameFlag)
    if not isOk then
        MyWelderDebugLog("subscribe fail:"..tostring(errmsg))
    end
    return isOk
end
...
function MqttRobot.triggerAddCurrentPose()
    return MqttRobot.publish("Weld/addCurrentPoint","addCurrentPoint")
end
