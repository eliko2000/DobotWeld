...
-------------------------------------------------------------------------------------------------------------------
--【业务实现接口】-------------------------------------------------------------------------------------------------
local CloosWelder = ImplementWelder:new()
CloosWelder.__index = CloosWelder

-------------------------------------------------------------------------------------------------------------------
--【对内不公开接口】---------------------------------------------------------------------------------------------
function CloosWelder:doStickRelease()
    return true --不支持粘丝解除功能默认返回true
end

-------------------------------------------------------------------------------------------------------------------
--【对外公开接口】-------------------------------------------------------------------------------------------------
function CloosWelder:new(globalParamObj,welderName)
    local o = ImplementWelder:new()
    o.welderName = welderName
    o.globalParamObject = globalParamObj
    o.welderParamObject = WelderParameter:new("CloosWelder")
    setmetatable(o,self)
    return o
end

function CloosWelder:getSupportParams()
    local cfg={}
...
function CloosWelder:connect()
    local ioParams = self:getWelderParamObject():getIOStreamParam()
    if "analogIO" == ioParams.name then
        self.welderControlObject = CloosWelderControlDAnalogIO:new(self)
    else
...
function CloosWelder:isJobMode()
    return true --这款焊机模拟量是通过job模式控制
end
