...
-------------------------------------------------------------------------------------------------------------------
--【业务实现接口】-------------------------------------------------------------------------------------------------
local ESABChinaWelder = ImplementWelder:new()
ESABChinaWelder.__index = ESABChinaWelder

-------------------------------------------------------------------------------------------------------------------
--【对内不公开接口】---------------------------------------------------------------------------------------------
function ESABChinaWelder:doStickRelease()
    return true --不支持粘丝解除功能默认返回true
end

-------------------------------------------------------------------------------------------------------------------
--【对外公开接口】-------------------------------------------------------------------------------------------------
function ESABChinaWelder:new(globalParamObj,welderName)
    local o = ImplementWelder:new()
    o.welderName = welderName
    o.globalParamObject = globalParamObj
    o.welderParamObject = WelderParameter:new("ESABChinaWelder")
    setmetatable(o,self)
    return o
end

function ESABChinaWelder:getSupportParams()
    local cfg={}
    cfg.communicationType = {"deviceNet"} --参数值只能是 EnumConstant.ConstEnumIOStreamName 的值
...
function ESABChinaWelder:connect()
    local ioParams = self:getWelderParamObject():getIOStreamParam()
    if "deviceNet" == ioParams.name then
        self.welderControlObject = ESABChinaWelderControlDeviceNet:new(self)
...
function ESABChinaWelder:isJobMode()
    --[[
