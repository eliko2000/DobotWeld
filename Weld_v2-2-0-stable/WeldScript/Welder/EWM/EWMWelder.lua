...
-------------------------------------------------------------------------------------------------------------------
--【业务实现接口】-------------------------------------------------------------------------------------------------
local EWMWelder = ImplementWelder:new()
EWMWelder.__index = EWMWelder
...
-------------------------------------------------------------------------------------------------------------------
--【对外公开接口】-------------------------------------------------------------------------------------------------
function EWMWelder:new(globalParamObj,welderName)
    local o = ImplementWelder:new()
    o.welderName = welderName
    o.globalParamObject = globalParamObj
    o.welderParamObject = WelderParameter:new("EWMWelder")
    setmetatable(o,self)
    return o
end

function EWMWelder:getSupportParams()
    local cfg={}
    cfg.communicationType = {"modbus","deviceNet"} --参数值只能是 EnumConstant.ConstEnumIOStreamName 的值
    cfg.weldMode = {--参数只能是 EnumConstant.ConstEnumWelderWeldMode 的值
        "twoStepDCWeld", --两步常规直流焊（操作模式：两步，焊接方法：常规直流焊）
        "twoStepPulseWeld", --两步脉冲焊（操作模式：两步，焊接方法：脉冲焊）
        "specialTwoStepDCWeld", --特殊两步常规直流焊（操作模式：特殊两步，焊接方法：常规直流焊）
        "specialTwoStepPulseWeld" --特殊两步脉冲焊（操作模式：特殊两步，焊接方法：脉冲焊）
    } 
...
function EWMWelder:connect()
    local ioParams = self:getWelderParamObject():getIOStreamParam()
    if "modbus" == ioParams.name then
        self.welderControlObject = EWMWelderControlModbus:new(self)
    elseif "deviceNet" == ioParams.name then
        self.welderControlObject = EWMWelderControlDeviceNet:new(self)
...
function EWMWelder:isJobMode()
--[[只有job模式直接返回true
    local params = self:getWelderParamObject():getJobModeParam()
    if nil==params then return false end
    if type(params.jobId)~="number" then return false end
    if 0==params.jobId then return false end
]]--
    return true
end
