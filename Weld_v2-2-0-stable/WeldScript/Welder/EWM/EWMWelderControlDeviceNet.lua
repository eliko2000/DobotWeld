...
-------------------------------------------------------------------------------------------------------------------
--【业务实现接口】-------------------------------------------------------------------------------------------------
local EWMWelderControlDeviceNet = WelderControlDeviceNet:new()
EWMWelderControlDeviceNet.__index = EWMWelderControlDeviceNet
...
-------------------------------------------------------------------------------------------------------------------
--【对外公开接口】-------------------------------------------------------------------------------------------------
function EWMWelderControlDeviceNet:new(welderObj)
...
function EWMWelderControlDeviceNet:initWelder()
    --[[
    ...
    return self:innerUpdateHoldRegsValue_Address(0,0x20,function(oldV,newV) 
                                                        local v = oldV|(1<<5) --机器人准备好
                                                        v = v&(~(1<<4)) --灭弧
                                                        v = v&(~(1<<8)) --停止送丝
                                                        v = v&(~(1<<10)) --停止退丝
                                                        v = v&(~(1<<6)) --停止气检
...
function EWMWelderControlDeviceNet:notifyWelderThatRobotHasReady()
    local welderName = self.welderObject.welderName
...
function EWMWelderControlDeviceNet:notifyWelderThatRobotNotReady()
    local welderName = self.welderObject.welderName
    MyWelderDebugLog(welderName..":notifyWelderThatRobotNotReady->before write")
    return self:innerUpdateHoldRegsValue_Address(0,0xFFDF,function(oldV,newV)
                                                        local v = oldV&newV
...
function EWMWelderControlDeviceNet:isWelderReady()
...
function EWMWelderControlDeviceNet:setWeldCurrent(newVal)
    local welderName = self.welderObject.welderName
    if nil == newVal then
...
function EWMWelderControlDeviceNet:setArcStartCurrent(newVal)
    local welderName = self.welderObject.welderName
    if nil == newVal then
        local params = self.welderObject:getWelderParamObject():getNotJobModeParam()
        if #params.params < 1 or params.selectedId<1 or params.selectedId > #params.params then
...
function EWMWelderControlDeviceNet:setArcEndCurrent(newVal)
    local welderName = self.welderObject.welderName
    if nil == newVal then
        local params = self.welderObject:getWelderParamObject():getNotJobModeParam()
        if #params.params < 1 or params.selectedId<1 or params.selectedId > #params.params then
...
function EWMWelderControlDeviceNet:getWeldCurrent()
...
function EWMWelderControlDeviceNet:setWeldVoltage(newVal)
    local welderName = self.welderObject.welderName
    if nil == newVal then
        local params = self.welderObject:getWelderParamObject():getNotJobModeParam()
        if #params.params < 1 or params.selectedId<1 or params.selectedId > #params.params then
...
function EWMWelderControlDeviceNet:setArcStartVoltage(newVal)
    local welderName = self.welderObject.welderName
    if nil == newVal then
        local params = self.welderObject:getWelderParamObject():getNotJobModeParam()
        if #params.params < 1 or params.selectedId<1 or params.selectedId > #params.params then
...
function EWMWelderControlDeviceNet:setArcEndVoltage(newVal)
    local welderName = self.welderObject.welderName
    if nil == newVal then
        local params = self.welderObject:getWelderParamObject():getNotJobModeParam()
        if #params.params < 1 or params.selectedId<1 or params.selectedId > #params.params then
...
function EWMWelderControlDeviceNet:getWeldVoltage()
...
function EWMWelderControlDeviceNet:getWeldWireFeedSpeed()
...
function EWMWelderControlDeviceNet:setWeldMode(newVal)
    local welderName = self.welderObject.welderName
    local params = newVal
    if nil == params then
...
function EWMWelderControlDeviceNet:setJobId(newVal)
    local welderName = self.welderObject.welderName
    if nil == newVal then
...
function EWMWelderControlDeviceNet:setProcessNumber(newVal)
    local welderName = self.welderObject.welderName
    if nil == newVal then
        local params = self.welderObject:getWelderParamObject():getJobModeParam()
...
function EWMWelderControlDeviceNet:arcStart()
    local newVal = 0x0010
    local welderName = self.welderObject.welderName
...
function EWMWelderControlDeviceNet:isArcStarted()
    local welderName = self.welderObject.welderName
    local newVal = self:innerGetInRegsValue_Address(0) or {}
    if #newVal<1 then
...
function EWMWelderControlDeviceNet:arcEnd()
    local newVal = 0xFFEF
    local welderName = self.welderObject.welderName
...
function EWMWelderControlDeviceNet:isArcEnded()
    local welderName = self.welderObject.welderName
    local newVal = self:innerGetInRegsValue_Address(0) or {}
    if #newVal<1 then
...
function EWMWelderControlDeviceNet:hasEndArcByMannual()
    local welderName = self.welderObject.welderName
...
function EWMWelderControlDeviceNet:startWireFeed()
    local newVal = 0x0100
    local welderName = self.welderObject.welderName
...
function EWMWelderControlDeviceNet:stopWireFeed()
    local newVal = 0xFEFF
    local welderName = self.welderObject.welderName
...
function EWMWelderControlDeviceNet:startWireBack()
    local newVal = 0x0400
    local welderName = self.welderObject.welderName
...
function EWMWelderControlDeviceNet:stopWireBack()
    local newVal = 0xFBFF
    local welderName = self.welderObject.welderName
...
function EWMWelderControlDeviceNet:isStickRelease()
    local newVal = self:innerGetInRegsValue_Address(0) or {}
    ...
    return (newVal[1]&0x04)==0x00
end

function EWMWelderControlDeviceNet:startGasCheck()
    local newVal = 0x0040
    local welderName = self.welderObject.welderName
...
function EWMWelderControlDeviceNet:stopGasCheck()
    local newVal = 0xFFBF
    local welderName = self.welderObject.welderName
...
function EWMWelderControlDeviceNet:getWelderRunStateInfo()
    ...
    if (newVal[1]&0x04)==0x00 then info.wireState = 0
    else info.wireState = 1
    end
    return info
end

function EWMWelderControlDeviceNet:getWelderErrCode()
    local newVal = self:innerGetInRegsValue_Address(0) or {}
    if #newVal<1 then
        local welderName = self.welderObject.welderName
...
return EWMWelderControlDeviceNet