...
-------------------------------------------------------------------------------------------------------------------
--【业务实现接口】-------------------------------------------------------------------------------------------------
local FroniusWelder = ImplementWelder:new()
FroniusWelder.__index = FroniusWelder

-------------------------------------------------------------------------------------------------------------------
--【对内不公开接口】---------------------------------------------------------------------------------------------
function FroniusWelder:doStickRelease()
    return true --不支持粘丝解除功能默认返回true
end

-------------------------------------------------------------------------------------------------------------------
--【对外公开接口】-------------------------------------------------------------------------------------------------
function FroniusWelder:new(globalParamObj,welderName)
    local o = ImplementWelder:new()
    o.welderName = welderName
    o.globalParamObject = globalParamObj
    o.welderParamObject = WelderParameter:new("FroniusWelder")
    setmetatable(o,self)
    return o
end

function FroniusWelder:getSupportParams()
    local cfg={}
...
function FroniusWelder:connect()
    local ioParams = self:getWelderParamObject():getIOStreamParam()
    if "modbus" == ioParams.name then
        self.welderControlObject = FroniusWelderControlModbus:new(self)
    elseif "deviceNet" == ioParams.name then
        self.welderControlObject = FroniusWelderControlDeviceNet:new(self)
...
--目前只有job模式，所以固定写死
function FroniusWelder:isJobMode()
    return true
end

return FroniusWelder