...
-------------------------------------------------------------------------------------------------------------------
--【业务实现接口】-------------------------------------------------------------------------------------------------
local KemppiAXWelder = ImplementWelder:new()
KemppiAXWelder.__index = KemppiAXWelder

-------------------------------------------------------------------------------------------------------------------
--【对内不公开接口】---------------------------------------------------------------------------------------------
function KemppiAXWelder:doStickRelease()
    return true --不支持粘丝解除功能默认返回true
end

-------------------------------------------------------------------------------------------------------------------
--【对外公开接口】-------------------------------------------------------------------------------------------------
function KemppiAXWelder:new(globalParamObj,welderName)
    local o = ImplementWelder:new()
    o.welderName = welderName
    o.globalParamObject = globalParamObj
    o.welderParamObject = WelderParameter:new("KemppiAXWelder")
    setmetatable(o,self)
    return o
end

function KemppiAXWelder:getSupportParams()
    local cfg={}
...
function KemppiAXWelder:connect()
    local ioParams = self:getWelderParamObject():getIOStreamParam()
    if "modbus" == ioParams.name then
        self.welderControlObject = KemppiAXWelderControlModbus:new(self)
...
--目前只有job模式，所以固定写死
function KemppiAXWelder:isJobMode()
    return true
end

return KemppiAXWelder