--[[焊机实现接口，继承自`WelderControlModbus`]]--
...
local function innerVoltage_Locale2Welder(newVal)
    if newVal<-9.9 then newVal=-9.9 end
    if newVal>9.9 then newVal=9.9 end
    return (newVal+9.9)/19.8*200
    --[[
    if newVal<-50 then newVal=-50 end
    if newVal>50 then newVal=50 end
    return (newVal+50)/100*100]]--
end
--焊机电压转换为本地想要的电压值
...
-------------------------------------------------------------------------------------------------------------------
--【业务实现接口】-------------------------------------------------------------------------------------------------
local KolarcWelderControlModbus = WelderControlDeviceNet:new() --特殊一些，modbus的协议按照devicenet字节交换方式来处理
KolarcWelderControlModbus.__index = KolarcWelderControlModbus
...
-------------------------------------------------------------------------------------------------------------------
--【对外公开接口】-------------------------------------------------------------------------------------------------
function KolarcWelderControlModbus:new(welderObj)
...
function KolarcWelderControlModbus:initWelder()
    --[[
    ...
    local ret = self:innerUpdateHoldRegsValue_Address(0,0x02,function(oldV,newV)
...
function KolarcWelderControlModbus:notifyWelderThatRobotHasReady()
    local welderName = self.welderObject.welderName
...
function KolarcWelderControlModbus:notifyWelderThatRobotNotReady()
    local welderName = self.welderObject.welderName
...
function KolarcWelderControlModbus:isWelderReady()
...
function KolarcWelderControlModbus:setWeldCurrent(newVal)
    local welderName = self.welderObject.welderName
    if nil == newVal then
    ...
    return self:innerUpdateHoldRegsValue_Address(16,newVal,nil)
end

function KolarcWelderControlModbus:setArcStartCurrent(newVal)
    local welderName = self.welderObject.welderName
    if nil == newVal then
        local params = self.welderObject:getWelderParamObject():getNotJobModeParam()
        if #params.params < 1 or params.selectedId<1 or params.selectedId > #params.params then
...
function KolarcWelderControlModbus:setArcEndCurrent(newVal)
    local welderName = self.welderObject.welderName
    if nil == newVal then
        local params = self.welderObject:getWelderParamObject():getNotJobModeParam()
        if #params.params < 1 or params.selectedId<1 or params.selectedId > #params.params then
...
function KolarcWelderControlModbus:getWeldCurrent()
...
function KolarcWelderControlModbus:setWeldVoltage(newVal)
    local welderName = self.welderObject.welderName
    if nil == newVal then
        local params = self.welderObject:getWelderParamObject():getNotJobModeParam()
        if #params.params < 1 or params.selectedId<1 or params.selectedId > #params.params then
...
function KolarcWelderControlModbus:setArcStartVoltage(newVal)
    local welderName = self.welderObject.welderName
    if nil == newVal then
        local params = self.welderObject:getWelderParamObject():getNotJobModeParam()
        if #params.params < 1 or params.selectedId<1 or params.selectedId > #params.params then
...
function KolarcWelderControlModbus:setArcEndVoltage(newVal)
    local welderName = self.welderObject.welderName
    if nil == newVal then
        local params = self.welderObject:getWelderParamObject():getNotJobModeParam()
        if #params.params < 1 or params.selectedId<1 or params.selectedId > #params.params then
...
function KolarcWelderControlModbus:getWeldVoltage()
...
function KolarcWelderControlModbus:getWeldWireFeedSpeed()
...
function KolarcWelderControlModbus:setWeldMode(newVal)
    local welderName = self.welderObject.welderName
    local params = newVal
    if nil == params then
...
function KolarcWelderControlModbus:setJobId(newVal)
    local welderName = self.welderObject.welderName
    if nil == newVal then
        local params = self.welderObject:getWelderParamObject():getJobModeParam()
        newVal = params.jobId
    end
    if not (newVal>=0 and newVal<=99) then
        MyWelderDebugLog(welderName..Language.trLang("WELDER_JOBNUMBER_ERR").."jobId={0<=job<=99}:jobId="..tostring(newVal))
        return false
    end
...
function KolarcWelderControlModbus:arcStart()
    local newVal = 0x01
    local welderName = self.welderObject.welderName
...
function KolarcWelderControlModbus:isArcStarted()
    local welderName = self.welderObject.welderName
    local newVal = self:innerGetInRegsValue_Address(0) or {}
    if #newVal<1 then
...
function KolarcWelderControlModbus:arcEnd()
    local newVal = 0xFFFE
    local welderName = self.welderObject.welderName
...
function KolarcWelderControlModbus:isArcEnded()
    local welderName = self.welderObject.welderName
    local newVal = self:innerGetInRegsValue_Address(0) or {}
    if #newVal<1 then
...
function KolarcWelderControlModbus:startWireFeed()
...
function KolarcWelderControlModbus:stopWireFeed()
    local newVal = (~(1<<9))&0xFFFF
    local welderName = self.welderObject.welderName
...
function KolarcWelderControlModbus:startWireBack()
...
function KolarcWelderControlModbus:stopWireBack()
    local newVal = (~(1<<10))&0xFFFF
    local welderName = self.welderObject.welderName
...
function KolarcWelderControlModbus:startGasCheck()
...
function KolarcWelderControlModbus:stopGasCheck()
    local newVal = (~(1<<8))&0xFFFF
    local welderName = self.welderObject.welderName
...
function KolarcWelderControlModbus:getWelderRunStateInfo()
...
function KolarcWelderControlModbus:getWelderErrCode()
...
return KolarcWelderControlModbus