...
-------------------------------------------------------------------------------------------------------------------
--【业务实现接口】-------------------------------------------------------------------------------------------------
local LaserWelderControlDAnalogIO = WelderControlDAnalogIO:new()
LaserWelderControlDAnalogIO.__index = LaserWelderControlDAnalogIO
LaserWelderControlDAnalogIO.welderObject = nil --焊机对象，也就是`ImplementWelder`的派生类
...
-------------------------------------------------------------------------------------------------------------------
--【对外公开接口】-------------------------------------------------------------------------------------------------
function LaserWelderControlDAnalogIO:new(welderObj)
...
--电流电压设置不支持，所以不要执行，取而代之的是功率的设置
function LaserWelderControlDAnalogIO:setWeldCurrent(newVal)
    return true
end
function LaserWelderControlDAnalogIO:setArcStartCurrent(newVal)
    return true
end
function LaserWelderControlDAnalogIO:setArcEndCurrent(newVal)
    return true
end
function LaserWelderControlDAnalogIO:setWeldVoltage(newVal)
    return true
end
function LaserWelderControlDAnalogIO:setArcStartVoltage(newVal)
    return true
end
function LaserWelderControlDAnalogIO:setArcEndVoltage(newVal)
    return true
end

function LaserWelderControlDAnalogIO:arcStart()
...
function LaserWelderControlDAnalogIO:isArcStarted()
...
function LaserWelderControlDAnalogIO:arcEnd()
    local welderName = self.welderObject.welderName
    local params = self.welderObject:getWelderParamObject():getAnalogIOSignalParam()
...
function LaserWelderControlDAnalogIO:isArcEnded()
    local params = self.welderObject:getWelderParamObject():getAnalogIOSignalParam()
    local cbsp = params.controlBoxSignalParam or {}
    --如果没有配置起弧反馈检测信号，则认为该信号不需要检测
    if math.type(cbsp.arcStartCheck) ~= "integer" then
...
function LaserWelderControlDAnalogIO:getWelderRunStateInfo()
...
return LaserWelderControlDAnalogIO