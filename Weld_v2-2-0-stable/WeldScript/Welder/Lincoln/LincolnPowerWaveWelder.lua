...
-------------------------------------------------------------------------------------------------------------------
--【业务实现接口】-------------------------------------------------------------------------------------------------
local LincolnPowerWaveWelder = ImplementWelder:new()
LincolnPowerWaveWelder.__index = LincolnPowerWaveWelder

-------------------------------------------------------------------------------------------------------------------
--【对内不公开接口】---------------------------------------------------------------------------------------------
function LincolnPowerWaveWelder:doStickRelease()
    return true --不支持粘丝解除功能默认返回true
end

-------------------------------------------------------------------------------------------------------------------
--【对外公开接口】-------------------------------------------------------------------------------------------------
function LincolnPowerWaveWelder:new(globalParamObj,welderName)
    local o = ImplementWelder:new()
    o.welderName = welderName
    o.globalParamObject = globalParamObj
    o.welderParamObject = WelderParameter:new("LincolnWelderPowerWave")
    setmetatable(o,self)
    return o
end

function LincolnPowerWaveWelder:getSupportParams()
    local cfg={}
    cfg.communicationType = {"deviceNet","eip"} --参数值只能是 EnumConstant.ConstEnumIOStreamName 的值
...
function LincolnPowerWaveWelder:connect()
    local ioParams = self:getWelderParamObject():getIOStreamParam()
    if "deviceNet" == ioParams.name then
        self.welderControlObject = LincolnPowerWaveControlDeviceNet:new(self)
    elseif "analogIO" == ioParams.name then
        self.welderControlObject = WelderControlDAnalogIO:new(self)
    elseif "eip" == ioParams.name then
        self.welderControlObject = LincolnPowerWaveControlEIP:new(self)
    else
...
--目前只有job模式，所以固定写死
function LincolnPowerWaveWelder:isJobMode()
    return true
end

return LincolnPowerWaveWelder