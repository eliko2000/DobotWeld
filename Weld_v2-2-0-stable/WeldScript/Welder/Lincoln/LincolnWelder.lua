...
-------------------------------------------------------------------------------------------------------------------
--【业务实现接口】-------------------------------------------------------------------------------------------------
local LincolnWelder = ImplementWelder:new()
LincolnWelder.__index = LincolnWelder

-------------------------------------------------------------------------------------------------------------------
--【对内不公开接口】---------------------------------------------------------------------------------------------
function LincolnWelder:doStickRelease()
    return true --不支持粘丝解除功能默认返回true
end

-------------------------------------------------------------------------------------------------------------------
--【对外公开接口】-------------------------------------------------------------------------------------------------
function LincolnWelder:new(globalParamObj,welderName)
    local o = ImplementWelder:new()
    o.welderName = welderName
    o.globalParamObject = globalParamObj
    o.welderParamObject = WelderParameter:new("LincolnWelderDigiwave")
    setmetatable(o,self)
    return o
end

function LincolnWelder:getSupportParams()
    local cfg={}
...
function LincolnWelder:connect()
    local ioParams = self:getWelderParamObject():getIOStreamParam()
    if "deviceNet" == ioParams.name then
        self.welderControlObject = LincolnWelderControlDeviceNet:new(self)
...
--目前只有job模式，所以固定写死
function LincolnWelder:isJobMode()
    return true
end

return LincolnWelder