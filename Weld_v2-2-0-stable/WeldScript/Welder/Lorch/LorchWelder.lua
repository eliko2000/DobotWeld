...
-------------------------------------------------------------------------------------------------------------------
--【业务实现接口】-------------------------------------------------------------------------------------------------
local LorchWelder = ImplementWelder:new()
LorchWelder.__index = LorchWelder
...
-------------------------------------------------------------------------------------------------------------------
--【对外公开接口】-------------------------------------------------------------------------------------------------
function LorchWelder:new(globalParamObj,welderName)
    local o = ImplementWelder:new()
    o.welderName = welderName
    o.globalParamObject = globalParamObj
    o.welderParamObject = WelderParameter:new("LorchWelder")
    setmetatable(o,self)
    return o
end

function LorchWelder:getSupportParams()
    local cfg={}
...
function LorchWelder:connect()
    local ioParams = self:getWelderParamObject():getIOStreamParam()
    if "modbus" == ioParams.name then
        self.welderControlObject = LorchWelderControlModbus:new(self)
...
--目前只有job模式，所以固定写死
function LorchWelder:isJobMode()
    return true
end
