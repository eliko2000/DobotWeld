...
local function innerVoltage_Locale2Welder(newVal)
    ...
    if newVal>40 then newVal=40 end
    return newVal*10
end
--焊机电压转换为本地想要的电压值
...
-------------------------------------------------------------------------------------------------------------------
--【业务实现接口】-------------------------------------------------------------------------------------------------
local LorchWelderControlModbus = WelderControlModbus:new()
LorchWelderControlModbus.__index = LorchWelderControlModbus
...
-------------------------------------------------------------------------------------------------------------------
--【对外公开接口】-------------------------------------------------------------------------------------------------
function LorchWelderControlModbus:new(welderObj)
...
function LorchWelderControlModbus:initWelder()
    --[[
    ...
    return self:innerUpdateHoldRegsValue_Address(0,0x01,function(oldV,newV)
...
function LorchWelderControlModbus:notifyWelderThatRobotHasReady()
    local welderName = self.welderObject.welderName
...
function LorchWelderControlModbus:notifyWelderThatRobotNotReady()
    local welderName = self.welderObject.welderName
...
function LorchWelderControlModbus:isWelderReady()
    ...
    --bit5或bit6有一个为1即可，01100000b=0x60
    return (newVal[1]&0x0060)~=0x00
end

function LorchWelderControlModbus:setWeldCurrent(newVal)
    local welderName = self.welderObject.welderName
    if nil == newVal then
...
function LorchWelderControlModbus:setArcStartCurrent(newVal)
    local welderName = self.welderObject.welderName
    if nil == newVal then
        local params = self.welderObject:getWelderParamObject():getNotJobModeParam()
        if #params.params < 1 or params.selectedId<1 or params.selectedId > #params.params then
...
function LorchWelderControlModbus:setArcEndCurrent(newVal)
    local welderName = self.welderObject.welderName
    if nil == newVal then
        local params = self.welderObject:getWelderParamObject():getNotJobModeParam()
        if #params.params < 1 or params.selectedId<1 or params.selectedId > #params.params then
...
function LorchWelderControlModbus:getWeldCurrent()
...
function LorchWelderControlModbus:setWeldVoltage(newVal)
    local welderName = self.welderObject.welderName
    if nil == newVal then
        local params = self.welderObject:getWelderParamObject():getNotJobModeParam()
        if #params.params < 1 or params.selectedId<1 or params.selectedId > #params.params then
...
function LorchWelderControlModbus:setArcStartVoltage(newVal)
    local welderName = self.welderObject.welderName
    if nil == newVal then
        local params = self.welderObject:getWelderParamObject():getNotJobModeParam()
        if #params.params < 1 or params.selectedId<1 or params.selectedId > #params.params then
...
function LorchWelderControlModbus:setArcEndVoltage(newVal)
    local welderName = self.welderObject.welderName
    if nil == newVal then
        local params = self.welderObject:getWelderParamObject():getNotJobModeParam()
        if #params.params < 1 or params.selectedId<1 or params.selectedId > #params.params then
...
function LorchWelderControlModbus:getWeldVoltage()
...
function LorchWelderControlModbus:getWeldWireFeedSpeed()
...
function LorchWelderControlModbus:setWeldMode(newVal)
    local welderName = self.welderObject.welderName
    local params = newVal
    if nil == params then
...
function LorchWelderControlModbus:setJobId(newVal)
    local welderName = self.welderObject.welderName
    if nil == newVal then
...
function LorchWelderControlModbus:arcStart()
    local newVal = 0x0001
    local welderName = self.welderObject.welderName
...
function LorchWelderControlModbus:isArcStarted()
    local welderName = self.welderObject.welderName
    local newVal = self:innerGetInRegsValue_Address(0) or {}
    if #newVal<1 then
...
function LorchWelderControlModbus:arcEnd()
    local newVal = 0xFFFE
    local welderName = self.welderObject.welderName
...
function LorchWelderControlModbus:isArcEnded()
    local welderName = self.welderObject.welderName
    local newVal = self:innerGetInRegsValue_Address(0) or {}
    if #newVal<1 then
...
function LorchWelderControlModbus:hasEndArcByMannual()
    local welderName = self.welderObject.welderName
...
function LorchWelderControlModbus:clearError()
    local newVal = 0x8000
    local welderName = self.welderObject.welderName
...
function LorchWelderControlModbus:startWireFeed()
    local newVal = 0x0200
    local welderName = self.welderObject.welderName
...
function LorchWelderControlModbus:stopWireFeed()
    local newVal = 0xFDFF
    local welderName = self.welderObject.welderName
...
function LorchWelderControlModbus:startWireBack()
    local newVal = 0x0400
    local welderName = self.welderObject.welderName
...
function LorchWelderControlModbus:stopWireBack()
    local newVal = 0xFBFF
    local welderName = self.welderObject.welderName
...
function LorchWelderControlModbus:isStickRelease()
    local newVal = self:innerGetInRegsValue_Address(0) or {}
    ...
    return (newVal[1]&0x0080)==0x00
end

function LorchWelderControlModbus:startGasCheck()
    local newVal = 0x0100
    local welderName = self.welderObject.welderName
...
function LorchWelderControlModbus:stopGasCheck()
    local newVal = 0xFEFF
    local welderName = self.welderObject.welderName
...
function LorchWelderControlModbus:getWelderRunStateInfo()
    ...
    if (newVal[1]&0x80)==0x00 then info.wireState = 0
    else info.wireState = 1
    end
    return info
end

function LorchWelderControlModbus:getWelderErrCode()
...
return LorchWelderControlModbus