...
-------------------------------------------------------------------------------------------------------------------
--【业务实现接口】-------------------------------------------------------------------------------------------------
local MillerWelder = ImplementWelder:new()
MillerWelder.__index = MillerWelder

-------------------------------------------------------------------------------------------------------------------
--【对内不公开接口】---------------------------------------------------------------------------------------------
function MillerWelder:doStickRelease()
    return true --不支持粘丝解除功能默认返回true
end

-------------------------------------------------------------------------------------------------------------------
--【对外公开接口】-------------------------------------------------------------------------------------------------
function MillerWelder:new(globalParamObj,welderName)
    local o = ImplementWelder:new()
    o.welderName = welderName
    o.globalParamObject = globalParamObj
    o.welderParamObject = WelderParameter:new("MillerWelder")
    setmetatable(o,self)
    return o
end

function MillerWelder:getSupportParams()
    local cfg={}
    cfg.communicationType = {"eip"} --参数值只能是 EnumConstant.ConstEnumIOStreamName 的值
...
function MillerWelder:connect()
    local ioParams = self:getWelderParamObject():getIOStreamParam()
    if "eip" == ioParams.name then
        self.welderControlObject = MillerWelderControlEIP:new(self)
...
--目前只有job模式，所以固定写死
function MillerWelder:isJobMode()
    return true
end
